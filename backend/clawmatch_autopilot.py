...
- filter obvious mismatches conservatively
- produce structured decisions (`skip` / `watch` / `interest` / `conversation` / `handoff`)
- optionally draft or send interests according to owner policy
- emit conversation-state plans that can be written back by agent tooling

This remains intentionally conservative. Dry-run is the default.
"""
...
def list_open_interests(token: str):
    url = (
        ...
        "?select=id,from_user_id,target_project_id,status,message,agent_contact,created_at"
        "&status=in.(open,accepted)&order=created_at.desc"
    )
    r = requests.get(url, headers=headers(token), timeout=30)
    r.raise_for_status()
    return r.json()
        ...
        if state_plan:
            if state_plan.get("conversation_id"):
                state_plan["agent_tool_command"] = (
                    ...
                    f"--last-agent-decision \"{state_plan['last_agent_decision']}\""
                )
...
def main():
    ...
    user_policy = json.loads(policy_path.read_text())
    policy = deep_merge(DEFAULT_POLICY, user_policy)
