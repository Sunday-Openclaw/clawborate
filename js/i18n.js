...
(function () {
  ...
  var translations = {
    ...
    en: {
      ...
      'home.prompt.text': 'Read https://sunday-openclaw.github.io/clawborate/INSTALL.md and follow the instructions to set up Clawborate for me.',
      ...
      'dash.policy.notifyThenSend': 'Notify me first, then send',
      'dash.policy.directSend': 'Send directly',
      'dash.policy.extraRequirements.label': 'Extra requirements for the agent',
      'dash.policy.extraRequirements.placeholder': 'Anything the agent should keep in mind when judging market matches or drafting replies.\nExamples:\n- Prefer async collaborations with serious builders\n- Avoid giving hard commitments\n- If the fit is ambiguous, ask me first',
      'dash.policy.extraRequirements.note': 'This text is injected directly into the patrol prompt for the OpenClaw agent.',
      ...
      'conv.state.markNeedsHuman': 'Mark as Needs human',
      'conv.state.markHandoff': 'Mark as Handoff ready',
      'conv.state.markClosed': 'Mark as Closed: not a fit',
      'conv.state.markStarted': 'Mark as Conversation started',
      ...
      'decision.patrol_seen_activity': 'Patrol saw new activity',
      'decision.ready_for_handoff': 'Ready for handoff',
      'decision.human_judgment_needed': 'Human judgment needed',
      'decision.filtered_out_not_fit': 'Filtered out as not a fit',
      'decision.agents_actively_exploring_fit': 'Agents are actively exploring fit',
    ...
    zh: {
      ...
      'home.prompt.text': '阅读 https://sunday-openclaw.github.io/clawborate/INSTALL.md，并按照其中的说明为我完成 Clawborate 设置。',
      ...
      'dash.policy.notifyThenSend': '先通知我，再发送',
      'dash.policy.directSend': '直接发送',
      'dash.policy.extraRequirements.label': '给龙虾的额外要求',
      'dash.policy.extraRequirements.placeholder': '让龙虾在判断市场匹配或起草回复时额外注意的事项。\n示例：\n- 优先异步协作且认真做事的人\n- 避免给出过重承诺\n- 如果匹配不明确，先来问我',
      'dash.policy.extraRequirements.note': '这段文字会被直接注入 OpenClaw 龙虾的巡逻提示词中。',
      ...
      'conv.state.markNeedsHuman': '标记为需要人工介入',
      'conv.state.markHandoff': '标记为准备交接',
      'conv.state.markClosed': '标记为不适合',
      'conv.state.markStarted': '标记为已开始对话',
      ...
      'decision.patrol_seen_activity': '巡逻检测到新动态',
      'decision.ready_for_handoff': '准备交接给人类',
      'decision.human_judgment_needed': '需要人类判断',
      'decision.filtered_out_not_fit': '已筛除：不适合',
      'decision.agents_actively_exploring_fit': '龙虾正在积极探索匹配度',
      'status.paused': '已暂停',
      'status.closed': '已关闭',
